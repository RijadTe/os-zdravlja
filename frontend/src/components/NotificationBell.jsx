...
const NotificationBell = () => {
  ...
  // ============================================================
  // 🔥 INICIJALIZACIJA - SAMO JEDNOM (SVAKIH 4 SATA)
  // ============================================================
  useEffect(() => {
    ...
    if (userEmail && !fetchingRef.current) {
      ...
      // 🔥 POSTAVI INTERVAL - SVAKIH 4 SATA
      ...
      intervalRef.current = setInterval(() => {
        if (userEmail && isMounted.current) {
          console.log(`🔄 Osvježavam notifikacije (${new Date().toLocaleTimeString()})`);
          fetchNotifikacije(userEmail);
          generatePreporuke(userEmail);
        }
      }, 4 * 60 * 60 * 1000); // 4 sata = 14400000 ms
    }
  ...
  const generatePreporuke = async (email) => {
    ...
    try {
      ...
      if (res.status === 200 && isMounted.current) {
        ...
        // 🔥 OSVJEŽI NOTIFIKACIJE NAKON GENERISANJA PREPORUKA
        await fetchNotifikacije(email);
      }
    } catch (error) {
            ...
            <div className="sticky bottom-0 bg-white dark:bg-gray-800 p-2 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-gray-400 dark:text-gray-500">
                  {t('notification.refresh_every') || 'Osvježava se svakih 4 sata'}
                </span>
                <button
                  onClick={() => setIsOpen(false)}
