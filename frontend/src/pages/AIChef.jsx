...
import { shouldShowAds, getAdUnitId } from '../config/ads.config';
...
const AIChef = () => {
  ...
  // ============================================================
  // 🔥 NATIVE SPEECH RECOGNITION - VERZIJA 7.0.1 (RADI NA ANDROIDU)
  // ============================================================
  const handleNativeVoiceSearch = async () => {
    ...
    try {
      // 🔥 DIREKTAN PRISTUP - ne koristi import() (puca u produkcijskom buildu)
      const SpeechRecognition = window.Capacitor?.Plugins?.SpeechRecognition;

      if (!SpeechRecognition) {
        console.error('❌ SpeechRecognition plugin nije dostupan');
        console.log('Dostupni pluginovi:', Object.keys(window.Capacitor?.Plugins || {}));
      ...
      console.log('✅ SpeechRecognition plugin pronađen');

      // 🔥 VERZIJA 7: available() vraća { available: true }
      const availableResult = await SpeechRecognition.available();
      console.log('📋 available():', availableResult);
      ...
      // 🔥 VERZIJA 7: checkPermissions()
      const permCheck = await SpeechRecognition.checkPermissions();
      console.log('📋 checkPermissions():', permCheck);

      if (permCheck?.speechRecognition !== 'granted') {
        const permReq = await SpeechRecognition.requestPermissions();
        console.log('📋 requestPermissions():', permReq);

        if (permReq?.speechRecognition !== 'granted') {
      ...
      // 🔥 KLJUČNO: start() s popup: true (bez ovoga puca s "No match")
      const result = await SpeechRecognition.start({
        language: 'hr-HR',
        maxResults: 1,
        partialResults: false,
        popup: true,
        popupTitle: 'Izgovorite sastojke'
      });
      ...
      let errorMessage = t('aichef.voice.error');

      if (error.message?.includes('No match') || error.message?.includes("Didn't understand")) {
        errorMessage = t('aichef.voice.not_recognized');
      } else if (error.message?.includes('not available') || error.message?.includes('NOT_AVAILABLE')) {
        errorMessage = 'Glasovna pretraga nije dostupna na ovom uređaju';
      } else if (error.message?.includes('permission') || error.message?.includes('PERMISSION')) {
        errorMessage = 'Dozvola za mikrofon nije data';
      } else if (error.message?.includes('timeout') || error.message?.includes('TIMEOUT')) {
        errorMessage = 'Vrijeme za glasovnu pretragu je isteklo';
      }
  ...
  // ============================================================
  // 🔥 ADMOB NATIVE - SAMO AKO JE NATIVE
  // ============================================================
  const showNativeAd = () => {
    return new Promise(async (resolve) => {
      try {
        if (!isNative) {
          console.warn('⚠️ Native AdMob nije dostupan');
          resolve(false);
          return;
        }
        ...
        // 🔥 Direktan pristup (ne import)
        const AdMob = window.Capacitor?.Plugins?.AdMob;

        if (!AdMob) {
          console.error('❌ AdMob plugin nije dostupan');
        ...
        AdMob.addListener('onAdFailedToShow', () => {
          setIsVideoAdPlaying(false);
          resolve(false);
        });
  ...
  const showWebAd = () => {
    return new Promise((resolve) => {
      ...
      if (!shouldShowAds()) {
        let seconds = 0;
        setPoruka(t('aichef.messages.simulated_ad'));
        setPorukaType('info');
        const interval = setInterval(() => {
          seconds++;
          setPoruka(`⏳ Simulacija reklame: ${seconds}s`);
          if (seconds >= 3) {
      ...
      setPoruka('📢 Učitavam video reklamu...');
      setPorukaType('info');
      ...
      const pubId = adUnitId.split('/')[0];
      const slotId = adUnitId.split('/')[1];
      ...
      const timer = setInterval(() => {
        ...
        if (adDisplayed || adSenseLoaded) {
          if (seconds <= 5) {
            setPoruka(`⏳ Gledate reklamu: ${seconds}s`);
          }
  ...
  // ============================================================
  // 🔥 showVideoAd - isNative je boolean!
  // ============================================================
  ...
  const handleUnlockWithVideo = async () => {
    if (user?.premium) {
      setPoruka(t('aichef.messages.premium_access'));
      setPorukaType('info');
      setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 3000);
      return;
    }
    ...
    if (isVideoAdPlaying) {
      setPoruka(t('aichef.messages.ad_already_playing'));
      setPorukaType('warning');
      setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 3000);
      return;
    }
    ...
    if (videoAdCount >= maxVideoAds) {
      setPoruka(t('aichef.messages.video_ad_limit_reached', { maxVideoAds }));
      setPorukaType('warning');
      setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 4000);
      return;
    }
    ...
    if (dailyLimit.preostalo > 0) {
      setPoruka(t('aichef.messages.daily_limit_remaining', { remaining: dailyLimit.preostalo }));
      setPorukaType('success');
      setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 4000);
      return;
    }
    ...
    if (dailyLimit.broj_pretraga >= dailyLimit.max_pretraga) {
      setPoruka(t('aichef.messages.daily_limit_reached', { max_pretraga: dailyLimit.max_pretraga }));
      setPorukaType('warning');
      setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 4000);
      return;
    }
    ...
    try {
      ...
      if (!videoCompleted) {
        setPoruka(t('aichef.messages.video_ad_not_finished'));
        setPorukaType('error');
      ...
      setPoruka(t('aichef.messages.video_ad_unlocked', { remaining }));
      setPorukaType('success');
      setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 4000);
    } catch (error) {
      console.error('❌ Greška:', error);
  ...
  const handlePretraga = useCallback(async () => {
    ...
    if (slika && !user?.premium) {
      if (dailyLimit.preostalo <= 0) {
        setPoruka(t('aichef.messages.no_more_pictures'));
        setPorukaType('warning');
        setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 4000);
        return;
      }
    ...
    try {
      ...
      if (slika) {
        setPoruka('🔍 Prepoznajem sastojke sa slike...');
        setPorukaType('info');
        setProgress(15);
        setStatus('🖼️ OCR analiza slike...');
        ...
        try {
            ...
            'hrv+eng',
            {
              logger: m => {
                if (m.status === 'recognizing text') {
                  ...
                  setProgress(15 + progress * 0.15);
                  setOcrProgress(progress);
                  console.log(`📊 OCR progres: ${progress}%`);
                }
              }
            }
          );
          const ocrResult = result.data.text;

          finalText = ocrResult;
          console.log('📝 Prepoznat tekst:', finalText);

          if (!finalText || finalText.trim() === '') {
            setPoruka('❌ Nismo prepoznali sastojke na slici. Pokušajte sa boljom slikom ili unesite tekst ručno.');
            setPorukaType('error');
            setLoading(false);
            setProgress(0);
            setStatus('');
          ...
          setProgress(30);
          setPoruka('✅ Sastojci prepoznati! Tražim recepte...');

        } catch (ocrError) {
          console.error('❌ OCR greška:', ocrError);
          const fileName = slika.name?.split('.')[0] || 'nepoznato';
          if (fileName && fileName.length > 0) {
            finalText = fileName;
            console.log(`📝 Koristim naziv fajla kao fallback: ${fileName}`);
            setPoruka(`⚠️ Nismo prepoznali tekst, koristimo naziv fajla: "${fileName}"`);
            setPorukaType('warning');
            isImageProcessed = true;
          } else {
            setPoruka('❌ Greška pri prepoznavanju slike. Pokušajte ponovo.');
            setPorukaType('error');
            setLoading(false);
            setProgress(0);
            setStatus('');
      ...
      if (!response.ok) {
        let errorMessage = '❌ Greška na serveru. Pokušajte ponovo.';
        try {
          const errorData = await response.json();
          if (response.status === 429) {
            errorMessage = '❌ Previše zahteva. Pokušajte za minut.';
          } else if (response.status === 504) {
            errorMessage = '⏰ AI pretraga traje predugo. Pokušajte ponovo.';
          } else if (response.status === 503) {
            errorMessage = '🔧 AI usluga trenutno nije dostupna. Pokušajte kasnije.';
          } else {
      ...
      if (response.headers.get('X-Cache') === 'HIT') {
        setPoruka(t('aichef.messages.cache_hit', { elapsed }));
        setPorukaType('info');
      } else if (response.headers.get('X-Source') === 'database') {
        setPoruka(t('aichef.messages.database_result', { elapsed }));
        setPorukaType('success');
      } else if (response.headers.get('X-Source') === 'ai_generated') {
        setPoruka(t('aichef.messages.ai_generated', { elapsed }));
        setPorukaType('success');
      } else {
      ...
      console.error('❌ Greška:', error);
      clearInterval(statusInterval);
      setPoruka(`❌ ${error.message || t('aichef.errors.search_failed')}`);
      setPorukaType('error');
  ...
  // ============================================================
  // 🔥 DIREKTNA PRETRAGA (ZA GLASOVNU - SA FRONTEND OCR)
  // ============================================================
    ...
    try {
      ...
      if (slika) {
        ...
        try {
          ...
          if (!finalText || finalText.trim() === '') {
            setPoruka('❌ Nismo prepoznali sastojke na slici. Pokušajte ponovo.');
            setPorukaType('error');
            setLoading(false);
            setProgress(0);
            setStatus('');
          ...
          if (fileName && fileName.length > 0) {
            finalText = fileName;
            setPoruka(`⚠️ Koristim naziv fajla: "${fileName}"`);
            setPorukaType('warning');
            isImageProcessed = true;
          } else {
            setPoruka('❌ Greška pri prepoznavanju slike.');
            setPorukaType('error');
            setLoading(false);
            setProgress(0);
            setStatus('');
      ...
      if (!data || data.length === 0) {
        setPoruka('😕 Nema recepata za ove sastojke.');
        setPorukaType('warning');
        setRezultati([]);
        setLoading(false);
        setProgress(0);
  ...
  // ============================================================
  // 🔥 GLASOVNA PRETRAGA - ODABIR METODE
  // ============================================================
            ...
            <div
              onClick={() => {
                if (dailyLimit.preostalo > 0 || user?.premium) {
                  ...
                  setPoruka(t('aichef.messages.no_photos_watch_video'));
                  setPorukaType('warning');
                  setTimeout(() => { setPoruka(''); setPorukaType('info'); }, 4000);
                }
              }}
            ...
            <div className="mt-3 sm:mt-4 space-y-2">
              <div className="flex justify-between text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                <span className="flex items-center gap-1 sm:gap-2 truncate">
                  ...
                  {status}
                  {slika && ocrProgress > 0 && ocrProgress < 100 && ` (OCR: ${Math.round(ocrProgress)}%)`}
                </span>
              ...
              <div className="flex justify-between text-[10px] sm:text-xs text-gray-400 dark:text-gray-500">
                ...
                <span className="truncate">
                  ...
                  {poruka && poruka.includes('Prepoznajem') && '🔍 OCR u tijeku...'}
                </span>
              </div>
            </div>
          )}
