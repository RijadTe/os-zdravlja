...
// ============================================================
// 🔥 ADS MANAGER SA PRAVIM ADMOB ZA NATIVE I ADSENSE ZA WEB
// ============================================================
...
export const createBannerAd = (isPremium) => {
  ...
  if (isNative) {
    // 🔥 NATIVE - PRAVI AdMob banner (preko Capacitor)
...
// ============================================================
// 🎬 VIDEO REKLAME - PRAVI ADMOB (NATIVE) ILI SIMULACIJA (WEB)
// ============================================================
...
export class RewardedAdManager {
  ...
  loadAd = async () => {
    try {
      if (isNative) {
        // 🔥 NATIVE - PRAVI AdMob (dinamički import)
        try {
          ...
          // 🔥 FALLBACK - ako AdMob nije dostupan (npr. test)
          adLog(`AdMob import error: ${importError.message}`, 'warn');
          adLog('Using AdMob mock fallback');
      ...
      try {
        if (isNative) {
          ...
          try {
            ...
            // 🔥 FALLBACK - ako AdMob nije dostupan
            adLog(`AdMob show error: ${importError.message}`, 'warn');
            adLog('Using AdMob mock fallback');
            
            // Simuliramo gledanje videa (2 sekunde)
            let seconds = 0;
            const interval = setInterval(() => {
              seconds++;
              adLog(`⏳ Gledate reklamu (mock): ${seconds}s`);
              if (seconds >= 2) {
                clearInterval(interval);
                adLog('🎉 AdMob rewarded ad (mock) completed');
                if (onReward) onReward();
                resolve(true);
              }
            }, 1000);
