...
app.post('/api/weekly-plan', async (req, res) => {
  try {
    const { 
     ...
     // ============================================================
    // 🔥 KEŠIRANJE - JEDAN BLOK (uklonjen duplikat)
    // ============================================================
    const today = datum || new Date().toISOString().split('T')[0];

    if (!force) {
      // Ako nije force → proveri keš
      ...
      if (existingPlan) {
        ...
        if (razlikaDana <= 7) {
          console.log('✅ [KEŠ] Plan već postoji (star ' + razlikaDana + ' dana), vraćam ga BEZ AI');
      ...
      // force → obriši stari plan i generiši novi
      console.log('🔄 [FORCE] Korisnik želi novi plan, brišem stari i generišem novi');
